...
一个面向 TauriTavern / SillyTavern 预设作者的 PromptManager 条目管理工具。它提供双栏预设工作台：左侧选择来源预设，右侧选择目标预设，可以搜索、过滤、复制、收藏、重排、启用/停用、比对差异和预览条目；两侧条目都可以互相拖入、编辑和暂存修改，只有点击“保存预设”才会写回对应 OpenAI 预设。

## 导入脚本

仓库发布后，可以在酒馆助手脚本导入下面这个文件：

- [酒馆助手脚本-预设管理.json](酒馆助手脚本-预设管理.json)

入口使用酒馆助手脚本自带的“预设管理”按钮，不会在页面右下角额外创建浮动按钮：

```javascript
import 'https://cdn.jsdelivr.net/gh/jerryzmtz/tauritavern-preset-manager@v2.22/dist/preset-manager/index.js';
```
...
1. 在酒馆助手脚本按钮区点击“预设管理”打开面板。
2. 左侧选择来源预设，右侧选择目标预设。复制、拖入、删除、排序和开关修改都会先停留在面板草稿里，点击“保存预设”后才写回对应 OpenAI 预设。
3. 开启“比对模式”后，面板只读，不会修改草稿、保存预设或生成备份。列表会高亮正文不同、仅来源、仅目标的条目；标题、角色、启用状态差异只显示小徽标。
4. 比对摘要里的“相同 / 正文不同 / 仅来源 / 仅目标 / 辅助差异”可以点击作为过滤条件，并且会和当前搜索框、角色/启用过滤条件同时生效。
5. 选中有匹配对象的条目时，右侧详情区会显示来源和目标的只读正文对照，方便直接确认差异内容。
